...
function woothemes_metabox_insert() {
    ...
    elseif($woo_metabox['type'] == 'upload') // So, the upload inputs will do this rather
        {    
                ...
                $uploaddir = WP_CONTENT_DIR . '/woo_uploads/' ;
                $loc = WP_CONTENT_URL .'/woo_uploads/';
   ...
   update_option('woo_upload_custom_errors',$errors);
}
}
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function(){
        ...
        <?php //Errors
        $upload_errors = get_option('woo_upload_custom_errors');
         ...
         foreach($upload_errors as $error)
            {
                 if($error['error'] == 'folder_not_created' && $error_shown != true){
                    ?>
                    jQuery('form#post').before('<div class="updated fade"><p>WooThemes: <strong>Oh No!</strong> We don\'t have the permissions to create the upload folder on your server. Please create it manually: <em>/wp-content/<strong>woo_uploads</strong></em>. Thanks!</strong></p></div>');
                    <?php
                $error_shown = true;
                }
                if($error['error'] == 'invalid_file' ){
                ?>
                 jQuery('form#post').before('<div class="updated fade"><p>WooThemes: <strong><?php echo $error['name']; ?></strong> is not a valid image file, please try another file.</p></div>');
                 <?php
                }
            }
